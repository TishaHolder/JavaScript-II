...
runners.forEach ( (runner, index) => {
    ...
    fullName[index] = runner.first_name + " " + runner.last_name;
...
// The event director would like the information of all generous donors. Return an array named largeDonations that contains information about the runners that have donations greater than $200 and log the result.

let largeDonations = [];

largeDonations = runners.filter ( runner => {
...
// Problem 2
// ==== Problem 2: Use .map() ====
// The event director needs to know how much each donation will be after tax is deducted. Deduct 20% tax from each donation and log the result.

let taxDonations = [];

taxDonations = runners.map( runner => {
   return (runner.donation - (runner.donation * 0.2));
...
// Problem 3
// ==== Problem 3: Use .reduce() ====
// Now that we know how much each donation will be after taxes. The after tax donations from the taxDonations array need to be tallied up and stored in an afterTaxDonations array.

let afterTaxDonations = [];

afterTaxDonations = taxDonations.reduce ( (accumulator, currentValue) => {
